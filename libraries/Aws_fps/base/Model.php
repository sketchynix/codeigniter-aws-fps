 ...
 */ 
class Amazon_FPS_Model extends CI_Driver
{
        ...
        foreach ($this->_fields as $fieldName => $field) {
            ...
            if (is_array($fieldType)) {
                ...
                if ($this->_isComplexType($fieldType)) {
                    $elements = $xpath->query("./a:$fieldName", $dom);
                    if ($elements->length == 1) {
						$fileName = str_replace('Amazon_FPS_Model_', '', $fieldType) . '.php';
						require_once APPPATH . 'libraries/Aws_fps/base/Model/' . $fileName;
